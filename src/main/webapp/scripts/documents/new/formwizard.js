...
formtowizard.prototype={
    ...
    loadsection:function(rawi, bypasshooks){
        ...
        if (i<this.sections.count && doload){ //if next section to show isn't the same as the current section shown
            ...
            if (this.setting.revealfx[0]=="slide"){
                ...
                }, this.setting.revealfx[1]) //animate fieldset wrapper's height to accomodate next section's height
                this.sections.$innerwrapper.stop().animate({
                    left:-i*this.maxfieldsetwidth
                }, this.setting.revealfx[1], function(){ //slide next section into view
            ...
            this.paginatediv.$status.text("Página "+(i+1)+" de "+this.sections.count) //update current page status text
            ...
            if (i==0) //hide "prev" link
            {
                ...
                this.paginatediv.$navlinks.eq(0).click(function(){
                    window.location.href=thiswizard.setting.home
                })
            }
            else if (i==this.sections.count-1) //hide "next" link
            {
                this.paginatediv.$navlinks.eq(2).css('visibility', 'hidden');
    ...
    validate:function(section){
        ...
        var validated=true, invalidtext=[]
        ...
        for (var i=0; i<elements.length; i++){
            ...
            else if (/(file)/.test(elements[i].type) && elements[i].value=="" && !elements[i].disabled){ //file elements - adicionado por Marcos Nunes
                invalidate(elements[i])
            }
            ...
            if (/(hidden)/.test(elements[i].type) && elements[i].value==""){ //hidden elements - adicionado por Marcos Nunes
                invalidate(elements[i])
            }
        ...
        if (!validated)
            ...
            for (var i=0; i<invalidtext.length; i++){
                var objError = $("#"+invalidtext[i]);
                if(!objError.hasClass("error")){
                    objError.addClass("error").focus();
                    objError.before("<div class='error'>Campo obrigatório.</div>");
                    
                    this.sections.$outerwrapper.stop().animate({
                        height: this.sections.$sections.eq(section).outerHeight()
                    }, this.setting.revealfx[1]) //animate fieldset wrapper's height to accomodate next section's height
                }
            }
        return validated
    ...
    init:function(setting){
        ...
        jQuery(function($){ //on document.ready
            ...
            var maxfieldsetwidth=$("#browser").outerWidth(); //variable to get width of widest fieldset.sectionwrap
            ...
            $sections.each(function(i){ //loop through $sections again
                ...
                if (setting.revealfx[0]=="slide"){
                    ...
                    }).appendTo($sectionswrapper_inner) //set fieldset position to "absolute" and move it to inside sectionswrapper_inner DIV
                }
                ...
                var $thestep=$('<div class="step disabledstep" />').data('section', i).html('Passo '+(i+1)+'<div class="smalltext">'+$section.find('legend:eq(0)').text()+'</div>').appendTo($stepsguide)
            ...
            var $paginatediv=$('<div class="formpaginate" style="overflow:hidden;"><span class="cancel" style="float:left">Cancelar</span> <span class="prev" style="float:left">Voltar</span> <span class="status">Step 1 of </span> <span class="next" style="float:right">Avançar</span></div>')
            ...
            thiswizard.paginatediv={
                $main:$paginatediv,
                $navlinks:$paginatediv.find('span.prev, span.next, span.cancel'),
